...
export function StatCard({ label, value, delta, icon, accent = '#38bdf8', loading }) {
    ...
    return (
        <div className="bg-[#0d1b2a] border border-[#1e2d3d] rounded-xl p-5 hover:border-[#2a3f55] transition-colors duration-200">
            ...
            <div className="text-2xl font-semibold text-white mb-0.5 tabular-nums tracking-tight">{value}</div>
...
export function Badge({ label, variant = 'slate' }) {
    return (
        <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-xs font-medium border ${BADGE[variant] ?? BADGE.slate}`}>
            {label}
        </span>
    );
}
...
export function Card({ children, className = '', padding = true }) {
    return (
        <div className={`bg-[#0d1b2a] border border-[#1e2d3d] rounded-xl ${padding ? 'p-5' : ''} ${className}`}>
            {children}
        </div>
    );
}
...
const BTN_VARIANTS = {
    ...
    ghost: 'border border-[#1e2d3d] hover:border-[#2a3f55] text-slate-400 hover:text-slate-200 hover:bg-white/[0.04]',
...
export function Input({ label, error, hint, className = '', ...props }) {
    return (
        <div className={className}>
            ...
            <input
                className={`w-full bg-[#060d1a] border rounded-lg px-3 py-2 text-sm text-slate-200 placeholder-slate-700 outline-none transition-colors duration-150 ${error
...
export function Textarea({ label, error, className = '', rows = 3, ...props }) {
    return (
        <div className={className}>
            ...
            <textarea rows={rows}
                className={`w-full bg-[#060d1a] border rounded-lg px-3 py-2 text-sm text-slate-200 placeholder-slate-700 outline-none transition-colors duration-150 resize-none ${error ? 'border-red-500/50 focus:border-red-400' : 'border-[#1e2d3d] focus:border-[#38bdf8]/50'
                    }`}
                {...props}
            />
            {error && <p className="text-xs text-red-400 mt-1">{error}</p>}
...
export function Modal({ open, onClose, title, subtitle, children, footer, maxWidth = 'max-w-xl' }) {
    if (!open) return null;
    return (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-start justify-center p-4 pt-20 overflow-y-auto"
            onClick={onClose}>
            <div className={`bg-[#0a1628] border border-[#1e2d3d] rounded-2xl w-full ${maxWidth} shadow-2xl`}
