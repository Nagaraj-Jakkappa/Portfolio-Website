...
import { Card, PageHeader, Badge, Btn, Input, Select, Textarea, Modal, Toggle, EmptyState, Spinner, Ic } from '../../components/admin/ui/ui';
...
const PAGE_SIZE = 8;
const EMPTY_FORM = { title: '', description: '', longDescription: '', techStack: '', imageUrl: '', liveUrl: '', githubUrl: '', featured: false, category: 'web', order: 0 };
...
function ProjectModal({ project, onClose, onCreate, onUpdate }) {
    ...
    const handleSave = async () => {
        ...
        setSaving(true);
        try {
            const payload = { ...form, techStack: form.techStack.split(',').map(s => s.trim()).filter(Boolean), order: Number(form.order) };
    ...
    return (
        <Modal open onClose={onClose}
            ...
            subtitle={editing ? 'Update the project details below' : 'Fill in the details for your new project'}
            footer={<>
            ...
            <div className="space-y-4">
                <Input label="Project Title *" value={form.title} onChange={e => set('title', e.target.value)} placeholder="e.g. Pothole Detection App" error={errors.title} />
                <Textarea label="Short Description *" value={form.description} onChange={e => set('description', e.target.value)} placeholder="One-two sentences shown on the portfolio card" error={errors.description} />
                <Input label="Tech Stack (comma-separated)" value={form.techStack} onChange={e => set('techStack', e.target.value)} placeholder="React, Node.js, MongoDB, TailwindCSS" />
                ...
                <Input label="Image URL" value={form.imageUrl} onChange={e => set('imageUrl', e.target.value)} placeholder="https://res.cloudinary.com/…" hint="Use Cloudinary free tier to host project screenshots" />
                <div className="grid grid-cols-2 gap-3">
                    <Select label="Category" value={form.category} onChange={e => set('category', e.target.value)}>
                        {['web', 'fullstack', 'ml', 'other'].map(c => <option key={c} value={c}>{c}</option>)}
                    </Select>
                    <Input label="Display Order" type="number" value={form.order} onChange={e => set('order', e.target.value)} min={0} />
                </div>
                <Toggle value={form.featured} onChange={v => set('featured', v)} label="Mark as Featured" />
            </div>
        </Modal>
    );
}
...
function DeleteConfirm({ onConfirm, onCancel }) {
    return (
        <div className="absolute right-0 top-9 z-20 bg-[#0a1628] border border-red-500/20 rounded-xl p-4 shadow-2xl w-52"
            onClick={e => e.stopPropagation()}>
            <p className="text-xs text-slate-300 leading-relaxed mb-3">Permanently delete this project?</p>
            <div className="flex gap-2">
                <Btn variant="ghost" size="sm" onClick={onCancel} className="flex-1 justify-center">Cancel</Btn>
                <Btn variant="danger" size="sm" onClick={onConfirm} className="flex-1 justify-center">Delete</Btn>
            </div>
        </div>
    );
}
            ...
            <td className="px-4 py-3">
                <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                        <p className="text-sm font-medium text-slate-200 truncate max-w-[160px]">{project.title}</p>
                        <p className="text-xs text-slate-600 truncate max-w-[160px]">{project.description?.slice(0, 48)}…</p>
                    </div>
                </div>
            </td>
            {/* Tech */}
            <td className="px-4 py-3">
                <div className="flex flex-wrap gap-1 max-w-[180px]">
                    {project.techStack?.slice(0, 3).map(t => (
                        <span key={t} className="text-xs px-1.5 py-0.5 bg-[#1e2d3d] text-slate-500 rounded">{t}</span>
                    ))}
                    {(project.techStack?.length ?? 0) > 3 && (
                        <span className="text-xs px-1.5 py-0.5 bg-[#1e2d3d] text-slate-700 rounded">+{project.techStack.length - 3}</span>
                    )}
                </div>
            </td>
            {/* Category */}
            <td className="px-4 py-3">
                <Badge label={project.category ?? 'web'} variant={CAT_VARIANT[project.category] ?? 'slate'} />
            </td>
            {/* Featured */}
            <td className="px-4 py-3">
                {project.featured
            ...
            <td className="px-4 py-3">
                <div className="flex items-center gap-1 relative">
                    ...
                    <div className="relative">
                        ...
                        {confirmOpen && (
                            <DeleteConfirm
                                onConfirm={() => { onDelete(project._id); setConfirmOpen(false); }}
...
export default function ProjectsPage() {
    ...
    const filtered = useMemo(() => {
        let list = [...projects];
        if (search) list = list.filter(p => p.title?.toLowerCase().includes(search.toLowerCase()) || p.techStack?.some(t => t.toLowerCase().includes(search.toLowerCase())));
    ...
    const resetFilters = () => { setSearch(''); setCatFilter('all'); setFeatFilter('all'); setPage(1); };
            ...
            <Card className="mb-4" padding={false}>
                <div className="flex flex-wrap items-center gap-3 p-4">
                    ...
                    <label className="flex items-center gap-2 bg-[#060d1a] border border-[#1e2d3d] focus-within:border-[#38bdf8]/40 rounded-lg px-3 py-2 flex-1 min-w-[200px] transition-colors cursor-text">
                        ...
                        {search && (
                            <button onClick={() => { setSearch(''); setPage(1); }} className="text-slate-700 hover:text-slate-400">
                    ...
                    <span className="text-xs text-slate-700 ml-auto tabular-nums">{filtered.length} result{filtered.length !== 1 ? 's' : ''}</span>
                </div>
            </Card>
                                ...
                                ? Array.from({ length: 5 }).map((_, i) => (
                                    <tr key={i} className="border-b border-[#1e2d3d]">
                                        {Array.from({ length: 6 }).map((_, j) => (
                                            <td key={j} className="px-4 py-3"><div className="h-8 bg-[#1e2d3d] rounded animate-pulse" /></td>
                                        ))}
                                    </tr>
                                ))
                                : paged.length === 0
                                    ? (
                                        <tr><td colSpan={6}>
                                            <EmptyState
                                                ...
                                                description={search ? `No matches for "${search}"` : 'Add your first project to get started.'}
                                                action={
                                                    search
                                                        ? <Btn variant="ghost" size="sm" onClick={resetFilters}>Clear filters</Btn>
                                                        : <Btn variant="primary" size="sm" onClick={() => setModal('create')}><Ic d={IC.plus} size={13} /> Add Project</Btn>
                                                }
                                            />
                                        </td></tr>
                                    )
