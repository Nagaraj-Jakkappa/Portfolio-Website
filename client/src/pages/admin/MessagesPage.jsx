...
import { Card, Badge, Btn, EmptyState, PageHeader, Ic, Spinner } from '../../components/admin/ui/ui';
...
function Avatar({ name, size = 8 }) {
    ...
    return (
        <div className={`w-${size} h-${size} rounded-full bg-gradient-to-br ${GRAD[i]} flex items-center justify-center text-xs font-bold text-white flex-shrink-0`}>
            {name?.[0]?.toUpperCase() ?? '?'}
        </div>
    );
}
...
function ListItem({ msg, selected, onClick }) {
    return (
        <button onClick={onClick}
            className={`w-full text-left px-4 py-3.5 border-b border-[#1e2d3d] relative transition-colors ${selected ? 'bg-[#38bdf8]/[0.06] border-l-2 border-l-[#38bdf8]' : 'hover:bg-white/[0.02]'
                }`}>
            {!msg.read && <div className="absolute right-4 top-4 w-2 h-2 bg-[#38bdf8] rounded-full" />}
            <div className="flex items-center gap-2.5 mb-1.5">
                <Avatar name={msg.name} size={7} />
                <div className="min-w-0">
                    <p className={`text-sm truncate ${msg.read ? 'text-slate-400 font-normal' : 'text-white font-semibold'}`}>
                        {msg.name}
                    </p>
            ...
            {msg.subject && (
                <p className={`text-xs mb-1 truncate pr-6 ${msg.read ? 'text-slate-600' : 'text-slate-400'}`}>
                    {msg.subject}
                </p>
            )}
...
function Detail({ msg, onMarkRead, onDelete }) {
    ...
    return (
        <div className="flex flex-col h-full">
            {/* Header */}
            <div className="p-6 border-b border-[#1e2d3d] flex-shrink-0">
                <div className="flex items-start justify-between gap-4 mb-4 flex-wrap">
                    <div>
                        ...
                        <div className="flex flex-wrap items-center gap-2 text-xs">
                            ...
                            <a href={`mailto:${msg.email}`} className="text-[#38bdf8] hover:underline">{msg.email}</a>
                            <span className="text-slate-700">·</span>
            ...
            <div className="flex-1 overflow-y-auto p-6">
                <div className="bg-[#060d1a] border border-[#1e2d3d] rounded-xl p-5 max-w-2xl">
                    <div className="flex items-center gap-3 mb-4 pb-4 border-b border-[#1e2d3d]">
            ...
            {/* Reply bar */}
            <div className="p-4 border-t border-[#1e2d3d] flex-shrink-0">
                <a href={`mailto:${msg.email}?subject=Re: ${encodeURIComponent(msg.subject || 'Your message')}`}
                    className="flex items-center gap-2 w-full bg-[#060d1a] border border-[#1e2d3d] hover:border-[#38bdf8]/40 rounded-xl px-4 py-3 text-sm text-slate-600 hover:text-slate-300 transition-colors">
                    <Ic d={IC.reply} size={14} />
                    <span>Reply to <span className="text-[#38bdf8]">{msg.email}</span>…</span>
        ...
        <div className="p-5 md:p-7 max-w-[1400px] mx-auto" style={{ height: 'calc(100vh - 4rem)' }}>
            ...
            <div className="grid md:grid-cols-[300px_1fr] gap-4" style={{ height: 'calc(100% - 64px)' }}>
                ...
                <Card padding={false} className="flex flex-col overflow-hidden">
                    {/* Toolbar */}
                    <div className="p-3 border-b border-[#1e2d3d] space-y-2 flex-shrink-0">
                        <label className="flex items-center gap-2 bg-[#060d1a] border border-[#1e2d3d] focus-within:border-[#38bdf8]/40 rounded-lg px-3 py-2 transition-colors cursor-text">
                        ...
                        <div className="flex gap-1 bg-[#060d1a] border border-[#1e2d3d] rounded-lg p-0.5">
                            {['all', 'unread', 'read'].map(f => (
                                <button key={f} onClick={() => setFilter(f)}
                                    className={`flex-1 py-1.5 rounded-md text-xs font-medium capitalize transition-all ${filter === f ? 'bg-[#38bdf8] text-white' : 'text-slate-600 hover:text-slate-300'}`}>
                    ...
                    <div className="flex-1 overflow-y-auto">
                        {loading
                            ? Array.from({ length: 5 }).map((_, i) => (
                                <div key={i} className="flex gap-3 p-4 border-b border-[#1e2d3d] animate-pulse">
                                    <div className="w-7 h-7 rounded-full bg-[#1e2d3d] flex-shrink-0" />
                                    <div className="flex-1 space-y-2">
                                        <div className="h-3 bg-[#1e2d3d] rounded w-1/2" />
                                        <div className="h-2.5 bg-[#1e2d3d] rounded w-3/4" />
                                    </div>
                                </div>
                            ))
                            : filtered.length === 0
                                ? <EmptyState icon={<Ic d={IC.inbox} size={20} />}
                                    title="No messages"
                                    description={search ? `No results for "${search}"` : filter === 'unread' ? "You're all caught up!" : 'No messages yet.'} />
                                : filtered.map(m => (
                                    <ListItem key={m._id} msg={m} selected={selectedId === m._id} onClick={() => handleSelect(m)} />
                                ))
                        }
                    </div>
                </Card>
                    ...
                    {selected
                        ...
                        : (
                            <div className="h-full flex flex-col items-center justify-center text-center p-8">
                                <div className="w-14 h-14 rounded-2xl bg-[#1e2d3d] flex items-center justify-center mb-4 text-slate-700">
                                    <Ic d={IC.mail} size={22} />
                                </div>
                                <h3 className="text-sm font-medium text-slate-500 mb-1">No message selected</h3>
                                <p className="text-xs text-slate-700">Click a message from the list to read it here.</p>
                            </div>
                        )
                    }
                </Card>
