    ...
    return (
        <Modal open onClose={onClose}
            ...
            <div className="space-y-4">
                <Input label="Title *" value={form.title} onChange={e => set('title', e.target.value)} placeholder="e.g., Python Programming" />
                <Input label="Organization *" value={form.organization} onChange={e => set('organization', e.target.value)} placeholder="e.g., Microsoft" />
                ...
                <div className="grid grid-cols-2 gap-3">
                    <Input label="Date" type="date" value={form.date ? form.date.slice(0, 10) : ''} onChange={e => set('date', e.target.value)} />
                    <Input label="Credential Link" value={form.link} onChange={e => set('link', e.target.value)} placeholder="https://..." />
                </div>
    ...
    return (
        <div className="bg-[#0d1b2a] border border-[#1e2d3d] rounded-xl p-5 hover:border-[#38bdf8]/50 transition-all group flex flex-col h-full">
            ...
            <div className="flex-grow">
                ...
                {cert.description && <p className="text-[11px] text-slate-500 line-clamp-2 mb-3 leading-relaxed">{cert.description}</p>}
            </div>

            <div className="flex items-center gap-2 pt-3 border-t border-[#1e2d3d] mt-auto">
                {cert.link && (
                    <a href={cert.link} target="_blank" rel="noreferrer" className="p-1.5 text-slate-400 hover:text-[#38bdf8] transition-colors">
                        <Ic d={IC.link} size={13} />
                    </a>
                )}
                <button onClick={() => onEdit(cert)} className="p-1.5 text-slate-400 hover:text-white transition-colors">
                ...
                <div className="ml-auto">
                    {confirmDelete ? (
                        <div className="flex gap-2">
                            ...
                            <button onClick={() => onDelete(cert._id)} className="text-[10px] text-red-400 font-bold">Yes</button>
                        </div>
                    ) : (
                        <button onClick={() => setConfirmDelete(true)} className="p-1.5 text-slate-400 hover:text-red-400 transition-colors">
    ...
    return (
        <div className="p-5 md:p-8 max-w-[1400px] mx-auto">
            ...
            <PageHeader
                ...
                action={<Btn variant="primary" onClick={() => setModal('create')}><Ic d={IC.plus} size={14} /> Add New</Btn>}
            />

            {loading ? (
                <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-8">
                    {[1, 2, 3, 4].map(i => <div key={i} className="h-48 bg-[#0d1b2a] rounded-xl animate-pulse border border-[#1e2d3d]" />)}
                </div>
            ) : certs.length === 0 ? (
                <div className="mt-12">
                    <EmptyState title="No certificates found" description="Start by adding your first certification." />
                </div>
            ) : (
                <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-8">
