            ...
            <div className="space-y-4">
                {notifications.map(notification => (
                    <div
                        ...
                        className={`group relative overflow-hidden rounded-2xl border p-5 transition-all duration-300 ${notification.read
                                ? 'bg-[#0f172a]/50 border-[#1e293b]'
                                : 'bg-sky-500/5 border-sky-500/40 shadow-[0_0_20px_rgba(14,165,233,0.05)]'
                            }`}
                    >
                        <div className="flex items-start gap-4">
                            <div className={`mt-2 h-2.5 w-2.5 rounded-full ${notification.read ? 'bg-slate-700' : 'bg-sky-400 animate-pulse'}`} />

                            <div className="flex-1">
                                <div className="flex justify-between items-start">
                                    <h3 className={`font-bold ${notification.read ? 'text-slate-400' : 'text-white'}`}>
                                        {notification.title}
                                    </h3>
                ...
                {notifications.length === 0 && (
                    <div className="text-center py-20 border-2 border-dashed border-slate-800 rounded-3xl">
                        <p className="text-slate-500 font-mono italic">"The silence is peaceful. No new alerts."</p>
                    </div>
                )}
            </div>
        </div>
